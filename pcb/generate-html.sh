...
qsv join person $TMPFILE person $UNIQUE_BIOS |
  qsv select title,start,person,personLabel,genderLabel,dob,dobPrecision,dod,dodPrecision,image > html/current.csv

# Generate HTML
erb country=$(jq -r .jurisdiction.name meta.json) csvfile=html/current.csv -r csv -T- pcb/index.erb > html/index.html

echo "No matches for:"
qsv join --left-anti title wikidata/wanted-positions.csv title html/current.csv
